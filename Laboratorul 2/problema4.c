...
int student_vector_add(struct student_vector *student_vector, struct student *student)
{
    if(student_vector->capacity == student_vector->count) {
        ...
        free(student_vector->students); student_vector->students = NULL;
...
int student_vector_remove(struct student_vector *student_vector, int index)
{
    ...
    for(i = index; i < student_vector->count; i += 1) {
        free(student_vector->students[i]); student_vector->students[i] = NULL;
...
int main()
{
    ...
    for(;;) {
        ...
        if('e' == command[0] && 'x' == command[1] && 'i' == command[2] && 't' == command[3] && '\x00' == command[4]) {
            for(int i = 0; i<student_vector->capacity; i+= 1) {
                if(student_vector->students[i] != NULL) {
                    free(student_vector->students[i]);
                    student_vector->students[i] = NULL;
                }
            }

            free(command); command = NULL;
            free(student_vector->students); student_vector->students = NULL;
            free(student_vector); student_vector = NULL;
            return 0;
        }
        // list
        ...
        // add
            ...
            if (NULL == new_student) {
                free(new_student); new_student = NULL;
                printf("ERROR! Out of memory!\n");
                continue;
            }
            ...
            if(NULL == new_student->name || NULL == new_student->surname) {
                printf("ERROR! Out of memory!\n");
                free(new_student->name); new_student->name = NULL;
                free(new_student->surname); new_student->surname = NULL;
                free(new_student); new_student = NULL;
                continue;
            }
